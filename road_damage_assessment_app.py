...
#     # --- REMOVE ROI guide/virtual lines (blue and yellow horizontal lines) ---
...
#     # Remove HUD text (no text in output window)
...
while True:
    ...
    # -------------------- NEW: apply 5-frame anticipation --------------------
    # Use the planner's produced sequence to look 5 steps ahead and, if feasible,
    # move toward that future column now (while respecting MAX_COL_CHANGE_PER_FRAME).
    lookahead_frames = 5
    if planned_seq and len(planned_seq) > 0:
        future_idx = min(lookahead_frames, len(planned_seq) - 1)
        try:
            target_future_col = int(planned_seq[future_idx])
        except Exception:
            target_future_col = chosen_col
    else:
        target_future_col = chosen_col

    feasible_range = list(range(max(0, yellow_col - MAX_COL_CHANGE_PER_FRAME),
                                min(num_parts, yellow_col + MAX_COL_CHANGE_PER_FRAME + 1)))

    # prefer target_future_col if it's reachable in one step, otherwise fall back to chosen_col
    if target_future_col in feasible_range:
        applied_col = target_future_col
    else:
        applied_col = chosen_col

    # apply limited step (keeps movement car-feasible)
    step = int(np.clip(applied_col - yellow_col, -MAX_COL_CHANGE_PER_FRAME, MAX_COL_CHANGE_PER_FRAME))
    yellow_col = int(yellow_col + step)
    # -----------------------------------------------------------------------
    ...
    # Draw yellow trail
    for hx, hy in yellow_history[-60:]:
        # we removed the dot requirement per your request; the trail can remain if you want it
        cv2.circle(processed, (hx, hy), 2, (0, 200, 200), -1, lineType=cv2.LINE_AA)
